      ...
      body: ListView.builder(
          ...
          itemBuilder: (context, index) {
            return Card(
              ...
              subtitle:
                  Text('There are 24 hours left until this event starts!'),
            ));
          }),
    );
  }
...
void Notify(DateTime time, String name, String eventImage) async {
  ...
  DateTime initialtime = new DateTime(
      time.year,
      time.month,
      time.subtract(new Duration(days: 1)).day,
      time.hour,
      time.minute,
      time.second,
      time.microsecond);
  DateTime now = new DateTime.now();
  DateTime newtime = new DateTime(now.year, now.month, now.day, now.hour,
